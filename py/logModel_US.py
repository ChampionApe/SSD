...
class infHorizon:
	...
	@property
	def auxΓβ1(self):
		return sum( (self.db['βi']/(1+self.db['βi'])) * self.db['γi'] * self.aux_Prod)
	@property
	def auxΓβ2(self):
		return sum( self.db['γi'] / (1+self.db['βi']))

	def auxPen(self, τp, epsp, epsc = 1):
		return self.db['p']*τp/(epsp*self.db['p̄']+epsc*self.db['p'])

	def aux_Γs(self, τp, epsp, θp, epsc = 1):
		""" τp, Bp, θp, epsp are vectors of the same length"""
		return (1/(1+self.db['ξ'])) * self.auxΓβ1 / (1+self.db['αr']*self.auxPen(τp, epsp, epsc = epsc)*(epsc*θp+(epsp+epsc*(1-θp)/(1-self.db['γ0']))*self.auxΓβ2))
	...
	def EE_Υ(self, τ, τp, epsp, θp):
		return ( ((1-self.db['α'])*(1-τ))/(1-self.db['αr']*self.auxPen(τp, epsp)*θp*self.aux_Γs(τp, epsp, θp)))**((1+self.db['ξ'])/(1+self.db['α']*self.db['ξ']))
	...
	def aux_PEE_polObj(self, τ, eps, θ):
		return (self.db['γ0']*self.ω20*self.aux_PEE_HtM_old(τ, eps)+np.matmul(self.ω2i * self.db['γi'], self.aux_PEE_retirees(τ, eps, θ))
				+self.db['ν']*(self.db['γ0']*self.ω10*self.aux_PEE_HtM_young(τ)+np.matmul(self.ω1i * self.db['γi'], self.aux_PEE_workers(τ))))

	def aux_PEE_retirees(self, τ, eps, θ):
		x = self.db['αr']*self.db['p']*(eps+θ*self.aux_Prod.reshape(self.ni,1)+(1-θ)/(1-self.db['γ0']))/(eps*self.db['p̄']+self.db['p'])
		return -(1-self.db['α'])*self.db['ξ']/((1+self.db['α']*self.db['ξ'])*(1-τ))+x/(self.savingsSpread(τ, eps, θ))

	def aux_PEE_HtM_old(self, τ, eps):
		return -(1-self.db['α'])*self.db['ξ']/((1+self.db['α']*self.db['ξ'])*(1-τ))+1/τ

	def aux_PEE_HtM_young(self, τ):
		return ((self.db['α']*(1+self.db['ξ'])/(1+self.db['α']*self.db['ξ']))/(1-τ))*(1-self.db['β0']*(1+self.db['ξ'])/(1+self.db['α']*self.db['ξ']))-self.db['ξ']*(τ/(1-τ))*(1+self.db['ξ'])/(1+self.db['ξ']*τ)

	def aux_PEE_workers(self, τ):
		return -((1+self.db['ξ'])/(1+self.db['α']*self.db['ξ']))*(1/(1-τ))*(1+self.db['βi']*self.db['α']*(1+self.db['ξ'])/(1+self.db['α']*self.db['ξ'])).reshape(self.ni,1)

	def savingsSpread(self, τp, epsp, θp):
		return (self.db['βi']*self.db['ηi']**(1+self.db['ξ'])/(self.db['Xi']**self.db['ξ']*(1+self.db['βi'])*(1+self.db['ξ']))).reshape(self.ni,1)/self.aux_Γs(τp,epsp,θp)-self.db['αr']*self.auxPen(τp, epsp)*(θp*self.aux_Prod.reshape(self.ni,1)+(epsp+(1-θp)/(1-self.db['γ0']))/(1+self.db['βi'].reshape(self.ni,1)))
	...
	def aux_ESC_eps(self, τ, eps, θ):
		...
		return self.ω20*self.db['γ0']*self.aux_ESC_eps_O0(τ, eps)+np.matmul(self.ω2i * self.db['γi'], self.aux_ESC_eps_O(τ, eps, θ))

	def aux_ESC_θ(self, τ, eps, θ):
		""" Condition for optimal θ """
		return np.matmul(self.ω2i * self.db['γi'], (self.aux_Prod-1).reshape(self.ni,1) / (self.db['α'] * self.savingsSpread(τ,eps,θ)+(1-self.db['α'])*self.auxPen(τ,eps)*(θ*self.aux_Prod.reshape(self.ni,1)+1-θ)))

	def aux_ESC_eps_O0(self, τ, eps):
		return 1/eps-self.db['p̄']/(eps*self.db['p̄']+self.db['p'])

	def aux_ESC_eps_O(self, τ, eps, θ):
		return ((self.db['αr']*self.db['p']*τ)/(self.savingsSpread(τ,eps,θ)+self.db['αr']*self.auxPen(τ, eps)*(eps+(θ*self.aux_Prod.reshape(self.ni,1)+(1-θ)/(1-self.db['γ0'])))))*((self.db['p']-self.db['p̄']*(θ*self.aux_Prod.reshape(self.ni,1)+(1-θ)/(1-self.db['γ0'])))/(eps*self.db['p̄']+self.db['p'])**2)
