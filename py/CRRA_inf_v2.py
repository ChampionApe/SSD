...
class infHorizon:
	...
	def defaultInitials(self):
		...
		return {'ss': np.full(self.ni+1, 0.5),
				...
				'ESC_t': np.full(self.ns['ESC[t]'].len, 0.2),
				'ESCB_t': np.full(self.ns['ESC[t]'].len, 0.2),
				'steadyState_PEE': np.full(self.ngrid, .2)}

	def getx0(self, solve, i =None, t= None):
		x0 = self.x0[solve][t] if t else self.x0[solve]
		return x0 if i is None else x0[i]

	# def getx0(self, solve, i = None, t = None):
	# 	return x0vec(self.x0[solve] if t is None else self.x0[solve][t], i = i)
	...
	def steadyState_solp_ESC(self, τ, θ, eps, ν, s, **kwargs):
		...
		ss = self.solve_ss_ESC(τ, θ, eps, ν, x0_ss_ESC = None, **kwargs) # solve steady state on grids
	...
	def solve_steadyState_PEE(self, θ, eps, ν, s, x0_steadyState_PEE = None, tol_SA = None, iterMax = 100, tol_NK = 1e-5, path = True, **kwargs):
		""" Solve for the steady state PEE policy. 
			...
			kwargs::: are passed to the initial solve in the self.solve_ssVec - thus can be used to guide initial guess on ss. """
		if tol_SA:
			sol = self.steadyState_SA_PEE(noneInit(x0_steadyState_PEE, self.getx0('steadyState_PEE')), θ, eps, ν, s, tol = tol_SA, iterMax = iterMax, **kwargs)
			if tol_SA>tol_NK:
			...
			fullSol = optimize.root(lambda x: self.steadyState_eq_PEE(x, θ, eps, ν, s, **kwargs), noneInit(x0_steadyState_PEE, self.getx0('steadyState_PEE')), tol = tol_NK, **kwargs)
			assert fullSol['success'], f""" Couldn't identify steady state policy (self.solve_steadyState_PEE)"""
			sol = fullSol['x']
		return self.report_steadyState_PEE(sol, θ, eps, ν, s, path = path)

	def steadyState_SA_PEE(self, τ, θ, eps, ν, s, tol = 1e-5, iterMax = 100, **kwargs):
		""" Successive approximation of the policy function until tolerance is reached. """
		i = 0
		sol_p = self.steadyState_solp0(τ, θ, eps, ν, s, **kwargs) # initial solution guess
		sol = self.resampleSolution(self.solve_PEE_t(θ, eps, ν, θ, eps, ν, sol_p, ite_PEE_t = True), s)
		while i<iterMax:
	...
	def solve_PEEvec_t(self, θ, eps, ν, θp, epsp, νp, sol_p, ite_PEEvec_t = True, t = None, **kwargs):
		...
		τ = optimize.root(lambda τ: self.PEE_polObjVec_t(τ, θ, eps, ν, θp, epsp, νp, sol, sol_p), sol_p['τ'] if ite_PEEvec_t else self.getx0('PEE_t', t=t), **kwargs)
	...
	def solve_ESCvec_t(self, ν, νp, sol_p, ite_ESCvec_t = True, t = None, **kwargs):
		sol = self.ESC_precomputations(ν, sol_p)
		esc = optimize.root(lambda x: self.ESC_polObjVec_t(self(x,'τ'), self(x,'θ'), self(x,'eps'), ν, νp, sol, sol_p),  np.hstack([sol_p['τ'], sol_p['θ'], sol_p['eps']]) if ite_ESCvec_t else self.getx0('ESC_t',t =t), **kwargs)
	...
	def solve_ESCBvec_t(self, ν, νp, sol_p, ite_ESCBvec_t = True, t = None, **kwargs):
		sol = self.ESC_precomputations(ν, sol_p)
		esc = optimize.root(lambda x: self.ESCB_polObjVec_t(self(x,'τ'), self(x,'θ'), self(x,'eps'), ν, νp, sol, sol_p), np.hstack([sol_p['τ'], sol_p['θ_notBound'], sol_p['eps_notBound']]) if ite_ESCBvec_t else self.getx0('ESCB_t',t=t), **kwargs)
