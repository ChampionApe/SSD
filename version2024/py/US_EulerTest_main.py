...
class Model:
	...
	def defaultParameters(self):
		return {'α': .5, 'A': np.ones(self.T), 'ν': np.ones(self.T), 'ξ' : .35, 'ρ': 1.2, 'ω': 2,  # parameters
				'τ0': .158, 'RR0': 39.4/50.1, 'UShare0': 3.4/15.8, 'R0': 2.443, 'βpower': .5}
	...
	def initUS(self):
		...
		if 'βi' not in self.db:
			self.db['βi'] = self.simpleβi
		self.US_Xi()
		self.db.update(self.initSC(self.US_κ(), 'κ'))

	def US_β(self, β):
		return β*(self.db['ηi']**(self.db['βpower']))
		# return β*np.full(self.ni, self.db['p'])
	def US_βinv(self):
		return (self.db['βi']*(self.db['ηi']**(-self.db['βpower'])))[0]
