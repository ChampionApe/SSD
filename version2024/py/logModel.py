...
class infHorizon:
	...
	def __call__(self, x, name, ns = 'PEE', **kwargs):
		return self.ns[ns](x, name, **kwargs)

	def get(self, x, name, ns = 'PEE'):
		return self.ns[ns].get(x, name)

	def leadSym(self, symbol, lead = -1, opt = None):
		return self.ns['PEE'].getShift(symbol, lead, opt = noneInit(opt, {'useLoc':'nn'})) if isinstance(symbol, pd.Series) else pd.Series(symbol).shift(lead, fill_value=symbol[-1]).values
	...
	def aux_Γs(self, τp, epsilon, θ):
		return (1/(1+self.db['ξ'])) * self.auxΓβ1 / (1+((1-self.db['α'])/self.db['α'])*self.auxPen(τp, epsilon)*(θ +(1-θ)*self.auxΓβ2))

	def savingsRate(self, Θs, Θh):
		return Θs/((1-self.db['α'])*(Θh**(1-self.db['α'])))

	################ EE:
	def solve_EE(self, τ, τp, epsilon = None, θ = None):
		epsilon, θ = noneInit(epsilon, self.db['epsilon']), noneInit(θ, self.db['θ'])
		Υ = self.EE_Υ(τ, τp, epsilon, θ)
		return {'Υ': pd.Series(Υ, index = self.db['t'], name = ' Υ'),
				'Θs': pd.Series(self.EE_Θs(Υ = Υ, τp = τp, epsilon = epsilon, θ = θ), index = self.db['t'], name = 'Θs'),
				'Θh': pd.Series(self.EE_Θh(Υ = Υ), index = self.db['t'], name = 'Θh')}

	def EE_Υ(self, τ, τp, epsilon, θ):
		return np.power((1-self.db['α'])*(1-τ)*self.db['A'] / (1-((1-self.db['α'])/self.db['α'])*θ*self.auxPen(τp, epsilon)*self.aux_Γs(τp, epsilon, θ)), 
							(1+self.db['ξ'])/(1+self.db['α']*self.db['ξ']))

	def EE_Θs(self, Υ = None, τp = None, epsilon = None, θ = None):
		return Υ * self.aux_Γs(τp, epsilon, θ)
	...
	def updateAndSolve(self, x0 = None, **kwargs):
		...
		self.db.update(self.solve_PEE(self.db['τ'].values if 'τ' in self.db else None))
		return self.db
	...
	def solve_PEE(self, τ0 = None, epsilon = None, θ = None):
		...
		return {'τ': pd.Series(τ, index = self.db['t'], name = 'τ'), 
				'τ[t+1]': pd.Series(τp, index = self.db['t'], name = 'τ[t+1]')} | self.solve_EE(τ, τp, epsilon = epsilon, θ = θ)
	...
	def savingsSpread(self, τp, epsilon, θ):
		x1 = ((self.db['β'] * np.power(self.db['η'], 1+self.db['ξ'])/(np.power(self.db['X'], self.db['ξ'])*(1+self.db['β']))) / self.auxΓβ1).reshape(self.ni,1)
		return x1+((1-self.db['α'])/self.db['α'])*self.auxPen(τp, epsilon) * (θ   * (x1-self.aux_Prod.reshape(self.ni,1)) 
																			+(1-θ)* (x1*self.auxΓβ2-1/(1+self.db['β']).reshape(self.ni,1)))
	# Reporting functions:
	...
	@property
	def aux_Θsi(self):
		return pd.DataFrame((self.db['Θs'].values*self.savingsSpread(self.db['τ[t+1]'].values, self.db['epsilon'], self.db['θ'])).T, 
			index = self.db['t'], columns = self.db['i'])

	@property
	def aux_Θc1i(self):
		return pd.DataFrame((self.db['Υ'].values * (self.aux_Prod*(1-self.db['β']/((1+self.db['ξ'])*(1+self.db['β'])))).reshape(self.ni,1)+self.db['Θs'].values*((1-self.db['α'])/self.db['α']) * (1-self.db['θ'])*self.auxPen(self.db['τ[t+1]'].values, self.db['epsilon'])/((1+self.db['β']).reshape(self.ni,1))).T,
			index = self.db['t'], columns = self.db['i'])

	@property
	def aux_Θc2i(self):
		return pd.DataFrame((self.db['α']*self.db['A']*self.db['ν']*(self.db['Θh'].values**(1-self.db['α']))*(self.savingsSpread(self.db['τ'].values, self.db['epsilon'],self.db['θ'])+((1-self.db['α'])/self.db['α'])*self.auxPen(self.db['τ'].values, self.db['epsilon'])*(self.db['θ']*self.aux_Prod.reshape(self.ni,1)+(1-self.db['θ'])))).T,
			index = self.db['t'], columns = self.db['i'])

	@property
	def aux_Θc2pi(self):
		return pd.DataFrame((self.db['α']*self.leadSym(self.db['A']*self.db['ν']*(self.db['Θh'].values)**(1-self.db['α']))*np.power(self.db['Θs'].values/self.leadSym(self.db['ν']), self.power_s)*(self.savingsSpread(self.db['τ[t+1]'].values, self.db['epsilon'],self.db['θ'])+((1-self.db['α'])/self.db['α'])*self.auxPen(self.db['τ[t+1]'].values, self.db['epsilon'])*(self.db['θ']*self.aux_Prod.reshape(self.ni,1)+(1-self.db['θ'])))).T,
			index = self.db['t'], columns = self.db['i'])
	...
	def aux_Θ̃c1i(self):
		return pd.DataFrame((self.db['Υ'].values/((1+self.db['ξ'])*(1+self.db['β'])).reshape(self.ni,1) * (
							...
							(1-self.db['θ'])*((1-self.db['α'])/self.db['α'])*self.auxPen(self.db['τ[t+1]'].values, self.db['epsilon']) * self.auxΓβ1 / (1+((1-self.db['α'])/self.db['α'])*self.auxPen(self.db['τ[t+1]'].values, self.db['epsilon'])*(self.db['θ']+(1-self.db['θ'])*self.auxΓβ2))
	...
	@property
	def aux_Θc2pu(self):
		return pd.Series((1-self.db['α']) * self.leadSym(self.db['A']) * self.leadSym(self.db['ν']) * (self.leadSym(self.db['Θh']).values**(1-self.db['α'])) * np.power(self.db['Θs'].values/self.leadSym(self.db['ν']), self.power_s) * (self.db['χ2']/self.leadSym(self.db['ν'])+self.db['epsilon']*self.auxPen(self.db['τ[t+1]'].values, self.db['epsilon'])),
				index = self.db['t'])
